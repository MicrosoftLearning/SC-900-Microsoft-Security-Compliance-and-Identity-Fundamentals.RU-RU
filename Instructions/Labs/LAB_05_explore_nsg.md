...
1. Выполните вход с использованием учетных данных администратора.
    ...
    1. Если вы ранее вошли в систему, может потребоваться выполнить вторичную проверку подлинности в рамках MFA. Если вы еще не вошли в систему, вам может потребоваться завершить процесс регистрации MFA. Следуйте инструкциям на экране, чтобы настроить MFA.
...
1. На панели навигации** слева разверните узел **"Сеть"**, а затем выберите **"Параметры сети".  В основных разделах главного окна отображается сетевой интерфейс для виртуальной машины.  Обратите внимание, что рядом с элементом "Группа безопасности сети" нет ничего, так как для интерфейса не назначена группа безопасности сети.

1. Не закрывайте эту вкладку.
...
1. На открытой вкладке Azure *щелкните правой кнопкой мыши* ссылку **Главная** в верхней части страницы и выберите **Открыть ссылку в новой вкладке**, чтобы открыть другую страницу в службах Azure.
...
1. В центре страницы нажмите синюю кнопку **Создать группу безопасности сети**.  Или же выберите **+ Создать** в верхней части страницы групп безопасности сети.
...
1. Должна быть открыта страница обзора для только что созданной группы безопасности сети.  В противном случае на панели навигации слева выберите **Обзор**. В верхней части страницы под надписью «Основное» вы увидите некоторые основные сведения о созданной группе безопасности сети.  Следует отметить два момента: отсутствуют настраиваемые правила безопасности, и отсутствуют подсети и сетевые интерфейсы, связанные с этой группой безопасности сети.  Несмотря на отсутствие настраиваемых правил безопасности, существуют правила для входящего и исходящего трафика по умолчанию, которые включены в каждую группу безопасности сети, как показано на странице.  Просмотрите правила для входящего и исходящего трафика. Правила для входящего трафика по умолчанию запрещают весь входящий трафик, который поступает не из виртуальной сети или подсистемы балансировки нагрузки Azure.  Правила для исходящего трафика запрещают весь исходящий трафик, кроме трафика между виртуальными сетями и исходящего трафика в Интернет.

1. В области навигации слева на странице NSG-SC900 разверните **раздел "Параметры** ", а затем выберите **сетевые интерфейсы**.
    1. Выберите **Связать**.
    2. В поле выбора связей сетевого интерфейса щелкните **стрелку вниз**, выберите **sc900-winvmXXX**, а затем нажмите **ОК** в нижней части окна. Как только интерфейс будет связан с Группой безопасности сети, он будет отображаться в списке.  Теперь группа безопасности сети назначена сетевому интерфейсу виртуальной машины.

1. Вернитесь на вкладку **SC900-WinVM — Microsoft Azure** в браузере.  Обновите страницу. Рядом с элементом "Группа безопасности сети" теперь должно отображаться имя только что созданной группы безопасности сети.  Если вы по-прежнему не видите его, подождите еще минуту, а затем снова обновите страницу.

1. На панели навигации слева выберите **Подключить**. В главном окне рядом с номером порта 3389 выберите **Проверка доступа**. Функция проверки доступа отправляет сигналы (трафик) на используемый по умолчанию RDP-порт 3389 виртуальной машины, чтобы проверить, доступен ли он. Через некоторое время отобразится статус "Недоступен".  Так и должно быть, поскольку правило группы безопасности сети DenyAllInBound запрещает весь входящий трафик на виртуальную машину.
...
1. Выберите **проверка доступа** (убедитесь, что порт имеет значение 3389).  Состояние должно отображаться как "Доступно".  Если вы по-прежнему видите "Недоступен", обновите страницу и повторите попытку, это может занять несколько минут, чтобы новое правило входящего трафика было видно с помощью параметра проверки доступа.
...
1. Когда виртуальная машина в сеансе RDP запущена и работает, проверьте исходящее подключение с нее к Интернету.
    1. На открытой виртуальной машине выберите **Microsoft Edge**, чтобы открыть браузер. Так как это первый раз, когда вы открываете виртуальную машину и broswer, вам может потребоваться некоторые основные параметры.  
    1. Вам может потребоваться выбрать параметры конфиденциальности для вашего устройства. Оставьте значение по умолчанию и нажмите кнопку **"Принять**".  
    1. Может отображаться боковая панель для сетей.  Выберите **Нет**.
    1. Может появиться окно с надписью "Обзор веб-сайта с лучшим браузером в Windows", нажмите кнопку "Продолжить", нажмите **кнопку **"Пуск" без данных****, нажмите **кнопку "Подтвердить" и продолжить**, **а затем нажмите кнопку **"Подтвердить" и начать просмотр**.**
